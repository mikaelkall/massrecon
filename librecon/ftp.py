...
class Ftp:
    ...
    def __init__(self, hostname=''):
        ...
        if len(self.hostname) == 0:
            utils.puts('warning', 'No hostname set')
            return
    ...
    def _mirror_ftp_dir(ftp_handle, name, overwrite, guess_by_extension):
        ...
        for item in ftp_handle.nlst(name):
            if Ftp()._is_ftp_dir(ftp_handle, item, guess_by_extension):
                Ftp()._mirror_ftp_dir(ftp_handle, item, overwrite, guess_by_extension)
            else:
                Ftp()._download_ftp_file(ftp_handle, item, item, overwrite)
    ...
    def download_ftp_tree(ftp_handle, path, destination, overwrite=False, guess_by_extension=True):
        ...
        Ftp().mirror_ftp_dir(ftp_handle, path, overwrite, guess_by_extension)
    ...
    def run(self):
        ...
        Ftp().download_ftp_tree(f, './', self.ftp_dir)